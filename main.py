...
def addStaff(role, name, phone, email):
    ...
    stri = "insert into Staff(role, name, phone_no, mail) values('%s', '%s', '%s', '%s')"
...
def mainroot():
    ...
    def reserve():
        ...
        def booking():
            if fn.get() == 'First Name' or ln.get() == 'Last Name' or cn.get() == 'Contact Number *' or em.get() == 'Email' or nop.get() == 'Number of Persons *' or nod.get() == 'Number of Days of Stay *' or roomn.get() == 'Enter Room Number *':
                messagebox.showinfo(
                ...
                if temp[0] == 1:
                    messagebox.showwarning(
                        'Room is Reserved', 'Room number '+roomn.get()+' is Reserved')
                elif temp[1] == 1:
                    messagebox.showwarning(
                        'Room is under renovation!', 'Room number '+roomn.get()+' is under renovation')
                else:
                    connection = MySQLdb.connect(
                        host='localhost', database='hotel', user='root', password='meetsql11')
                    cursor = connection.cursor()
                    ...
                    args = (d1,
                            int(nod.get()), fn.get()+" "+ln.get(), int(nop.get()), int(roomn.get()))
                    try:
                        cursor.execute(stri % args)
                        connection.commit()
                    except (Exception) as error:
                    ...
                    if ask:
                        ...
                        with open(os.path.join(path, title), "w") as file1:
                            toFile = create_bill(d1, nod.get(), fn.get(),
                                                 ln.get(), nop.get(), roomn.get())
                            file1.write(toFile)
                        messagebox.showinfo(
        ...
        def unreserve():
            if roomn.get() == 'Enter Room Number *' or '':
                messagebox.showinfo(
                    'Incomplete', 'Please enter room number')
            else:
                connection = MySQLdb.connect(
                    host='localhost', database='hotel', user='root', password='meetsql11')
                cursor = connection.cursor()
